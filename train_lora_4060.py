#!/usr/bin/env python3
"""
Low-VRAM LoRA training script optimized for RTX 4060 (~8GB).
...
This script trains LoRA adapters attached to attention projection layers.
For temporal LoRA (AnimateDiff/Tune-A-Video) you must run with an adapted UNet that has temporal-attention modules;
the code includes detection heuristics to attach adapters to modules with .to_q attributes.

Usage:
 accelerate launch scripts/train_lora_4060.py --model_id runwayml/stable-diffusion-v1-5 --dataset_dir ./data/frames --output_dir ./lora_out --resolution 512
"""
import os
import argparse
import math
...
def main():
    ...
    for n, p in unet.named_parameters():
        # our adapters create parameters; mark them trainable
        if "lora" in n:
